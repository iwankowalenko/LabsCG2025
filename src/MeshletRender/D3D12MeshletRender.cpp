...
const wchar_t* D3D12MeshletRender::c_meshFilename = L"..\\Assets\\orange_cat_new.bin";
const wchar_t* c_texturePath = L"..\\Assets\\orange_cat.png";
...
void D3D12MeshletRender::OnUpdate()
{
    ...
    // Переключение мешлетов/текстур (false - режим текстур, true - режим мешлетов)
...
void D3D12MeshletRender::OnMouseDown(UINT8 button, int x, int y)
{
    if (button == 0)
        m_camera.OnMouseDown(x, y);
}

void D3D12MeshletRender::OnMouseUp(UINT8 button, int x, int y)
{
    if (button == 0)
        m_camera.OnMouseUp(x, y);
}

void D3D12MeshletRender::OnMouseMove(int x, int y)
{
...
void D3D12MeshletRender::PopulateCommandList()
{
    ...
    for (auto& mesh : m_model)
    {
        ...
        for (auto& subset : mesh.MeshletSubsets)
        {
            ...
            const UINT MAX_TG = 65535;
            for (UINT base = 0; base < subset.Count; base += MAX_TG)
            {
                const UINT count = min(MAX_TG, subset.Count - base);

                // если твой mesh shader использует subset.Offset как стартовый meshlet,
                // нужно сдвигать Offset для каждого батча:
                m_commandList->SetGraphicsRoot32BitConstant(1, subset.Offset + base, 1);
